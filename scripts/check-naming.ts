...
const CONFIG = {
  ...
  ignoreNames: new Set(["README.md", "LICENSE", "CONTRIBUTE.md", ".gitignore", ".prettierrc", ".prettierignore"]),
...
const isHugoName = (name: string) => CONFIG.hugoNames.has(name) || /^_index(?:\.[a-z0-9-]+)?\.md$/.test(name);
...
function checkFile(file: string): Problem[] {
  ...
  if (CONFIG.checks.imageFormat && under(file, CONFIG.imageRoots)) {
    ...
    if (!markdownBundle && ext && !forbiddenImage && !CONFIG.allowedImageExtensions.has(ext)) {
      add("image-format", `only ${[...CONFIG.allowedImageExtensions].join("/")} allowed here (found ${ext})`);
    }
  }
